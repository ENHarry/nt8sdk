...
namespace NinjaTrader.NinjaScript.Indicators
{
    ...
    /// - Lines: up, down
    /// </summary>
    public class OBOSIndicator : Indicator
    {
        #region Variables
        ...
        private EMA emaDown;             // EMA of down
...
namespace NinjaTrader.NinjaScript.Indicators
{
    ...
    {
        private OBOSIndicator[] cacheOBOSIndicator;
        public OBOSIndicator OBOSIndicator(int length)
        {
            return OBOSIndicator(Input, length);
        }

        public OBOSIndicator OBOSIndicator(ISeries<double> input, int length)
        {
            if (cacheOBOSIndicator != null)
                for (int idx = 0; idx < cacheOBOSIndicator.Length; idx++)
                    if (cacheOBOSIndicator[idx] != null && cacheOBOSIndicator[idx].Length == length && cacheOBOSIndicator[idx].EqualsInput(input))
                        return cacheOBOSIndicator[idx];
            return CacheIndicator<OBOSIndicator>(new OBOSIndicator() { Length = length }, input, ref cacheOBOSIndicator);
        }
    }
}
...
namespace NinjaTrader.NinjaScript.Strategies
{
    ...
    {
        public Indicators.OBOSIndicator OBOSIndicator(int length)
        {
            return indicator.OBOSIndicator(Input, length);
        }

        public Indicators.OBOSIndicator OBOSIndicator(ISeries<double> input, int length)
        {
            return indicator.OBOSIndicator(input, length);
        }
    }
